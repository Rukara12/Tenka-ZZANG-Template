...
export const LIMITS = {
  maxFrames: 400,        // 움짤 1개에서 받아들일 최대 프레임
  maxExportFrames: 300,  // 내보내기 프레임 상한

  // 움짤 하나가 메모리에 풀어 놓을 수 있는 픽셀 총량 (가로×세로×장수).
  //
  // 프레임은 ImageBitmap 으로 미리 풀어 둔다. 압축이 풀린 RGBA 라 1픽셀이 4바이트고,
  // 1920x1080 한 장이 7.9MB 다. 장수만 세는 상한으로는 이걸 못 막는다.
  //
  // 지금까지는 파일 크기가 방패였다. 1280x720 300프레임짜리 GIF 는 200MB 가 넘어서
  // 아무도 넣지 않는다. 같은 내용의 WebP 는 10MB 대다 — 사용자는 가벼운 파일이라고
  // 믿고 넣는데 앱은 1GB 를 잡는다. 그 방패가 없어졌으니 여기서 직접 막는다.
  //
  // 128M 픽셀 = 512MB. 지금도 640x480 400프레임 GIF 가 이 언저리(123M)까지 가므로,
  // 이 값은 기존에 잘 되던 것을 건드리지 않으면서 무한대만 잘라 낸다.
  maxAssetPixels: 128e6,

  // 예산을 맞추려 줄이더라도 이 아래로는 안 줄인다. 여기서 더 줄이면 쓸 수 없다.
  minFrameEdge: 360,
  minFontSize: 8,
  maxFontSize: 200,
